...
if (!isset($_GET['idMascota'])) {
    echo "<script>alert('ID de mascota no especificado'); window.location='miPerfil.php';</script>";
    exit;
}
...
if (isset($_POST['submitModMiMascota'])) {
    ...
    $historial = mysqli_real_escape_string($conexion, $_POST['historialModMiMascota']);
...
<section class="mx-extra">
  ...
  <form class="row g-3 mx-extra" action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="POST">
    ...
    <article class="mb-3 row">
      ...
      <aside class="col-sm-6">
        <input type="text" class="form-control" name="historialModMiMascota" value="<?php echo htmlspecialchars($historialData); ?>"  required>
      </aside>
    </article>
    <!--Boton-->
